...
energy_bill_table = [["Tariff name", "Daily import bill (£)", "Daily export bill (£)", "Total daily bill (£)"],
                     ...
                     [tariff_names[0], round(-np.sum(np.minimum(0, g_default) * i_1) / readings_per_hour, 2), 0, round(-np.sum(np.maximum(0, g_default) * e_1) / readings_per_hour, 2)]]

tariff_counter = 1 # skip the first tariff because it uses the existing battery control logic
...
for import_tariff, export_tariff in [(i_2, e_2),(i_3, e_3)]:
    ...
    energy_bill_table[tariff_counter] = [tariff_names[tariff_counter], round(import_energy_bill,2), round(export_energy_bill,2), round(energy_bill,2)]
...
# assign default inverter readings to the arrays
